...
func (w *Watcher) pollEvents(files map[string]os.FileInfo, evt chan fsnotify.Event,
	cancel chan struct{}) {
	w.mu.Lock()
	defer w.mu.Unlock()
	...
	for path, info := range files {
		...
		if oldInfo.ModTime() != info.ModTime() {
			...
			case <-cancel:
				return
			case evt <- fsnotify.Event{Name: path, Op: fsnotify.Write}:
			}
		}
		if oldInfo.Mode() != info.Mode() {
			...
			case <-cancel:
				return
			case evt <- fsnotify.Event{Name: path, Op: fsnotify.Chmod}:
			}
		}
	}
	...
	for path := range creates {
		...
		case <-cancel:
			return
		case evt <- fsnotify.Event{Name: path, Op: fsnotify.Create}:
		}
	}
	for path := range removes {
		...
		case <-cancel:
			return
		case evt <- fsnotify.Event{Name: path, Op: fsnotify.Remove}:
		}
	}
}
